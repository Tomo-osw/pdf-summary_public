...
def pdf_summary(filename, summary_number):
...
class ConvertPDF2text():
    ...
    def __init__(self, argv:list):
        ...
        """
        self.input_path = r''
        #self.input_path = r'seminar.pdf'
        self.output_path = settings.MEDIA_ROOT + '/text/{}.txt'.format(datetime.datetime.now().strftime("%m%d_%H%M_%S"))
...
def main(pdfid, summary_number):
    cnv = ConvertPDF2text(sys.argv[1:])
    cnv.input_path = settings.MEDIA_ROOT + '/pdf/' + pdfid
    cnv.convert_pdf_to_text()
    return pdf_summary(cnv.output_path, summary_number), cnv.output_path